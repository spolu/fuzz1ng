...
class Pool:
    ...
    def store(
        ...
        self._runs = self._runs[:self._runs_size]
...
class RunsDB:
    ...
    def store(
        ...
        if skip-path not in self._skip_pool:
            self.skip_pool[skip_path].store(input, coverage)

        count_path = coverage.count_path_list()[0]
        if count_path not in self._count_pool:
            self.count_pool[count_path].store(input, coverage)
